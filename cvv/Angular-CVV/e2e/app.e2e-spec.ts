...
describe('Starting tests for Angular-CVV', function() {
  ...
  it('website title should be Angular-CVV', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('Angular-CVV');
    })
  });

  it('network-name should be cvv@1.0.5',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('cvv@1.0.5.bna');
    });
  });

  it('navbar-brand should be Angular-CVV',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('Angular-CVV');
    });
  });
